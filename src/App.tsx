...
import {
  ...
  Plus, Minus, Menu, X, MessageCircle,
} from "lucide-react";
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-500 ${scrolled ? "py-2" : "py-4"}`}>
      <div className="mx-auto max-w-7xl px-4">
        <div className={`flex items-center justify-between rounded-full px-4 sm:px-6 py-2 transition-all duration-500 ${scrolled ? "glass shadow-soft" : "bg-transparent"}`}>
          <a href="#home" className="flex items-center">
            <img
              src={logo}
              alt="Renew+ Skin Clinic"
    ...
    <section id="book" className="py-24 md:py-36 bg-luxury relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-5xl px-4">
        ...
        <form onSubmit={onSubmit} className="glass rounded-[2rem] p-6 md:p-10 grid md:grid-cols-2 gap-4">
          ...
          <SelectField label="Preferred Treatment" value={form.treatment} onChange={v => setForm(f => ({ ...f, treatment: v }))} options={TREATMENTS.map(t => t.name).concat(["Not sure — recommend"])} />
        ...
        <div className="divide-y divide-border">
          {FAQS.map(([q, a], i) => (
            <div key={i} className="py-5">
              <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex items-center justify-between text-left cursor-pointer">
